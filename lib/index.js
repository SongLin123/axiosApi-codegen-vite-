...
(async function main() {
  ...
  paths = [].concat(...(0, _lodash.default)(oldpaths).keys().map(item => {
    ...
    return (0, _lodash.default)(inter).keys().map(type => {
      ...
      let routes = item.split('/').filter(t => t.search(/\{/) ? t : '');
      return {
        tags: func.tags,
        request: {
          fullpath: item,
  ...
  Array.from(module.keys()).forEach(async key => {
    await (0, _render.renderModule)(key.dirname, key.filename, module.get(key), targetPath);
  });

  if (hasEntryService) {
    await (0, _utils.removeTar)(entryServiceTargetPath + 'index.js', ...basePath);
    await (0, _utils.removeTar)(entryServiceTargetPath + 'service.js', ...basePath);
    await (0, _utils.removeTar)(entryServiceTargetPath + 'tools.js', ...basePath);
    (0, _render.renderEntry)(entryServiceTargetPath);
  }
})();